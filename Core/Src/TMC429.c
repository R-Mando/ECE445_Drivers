...
void ReadWrite429(uint8_t *Read, uint8_t *Write)
{
	...
	enableChipSelect();
	Read[0] = ReadWriteSPI(&hspi1, Write[0], FALSE);
	Read[1] = ReadWriteSPI(&hspi1, Write[1], FALSE);
	Read[2] = ReadWriteSPI(&hspi1, Write[2], FALSE);
	Read[3] = ReadWriteSPI(&hspi1, Write[3], TRUE);
}
...
{
	SPI_HandleTypeDef hspi1;
	return ReadWriteSPI(&hspi1, 0x01, TRUE);
}
